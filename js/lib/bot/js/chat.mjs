...
const chat = (question, noneedTime = false) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if(question.data?.delay){
      setTimeout(() => {
        sleep(question.data.delay)
        resolve()
      }, (infoModal._isShown || infoModal._isTransitioning)?700:200)
    }else{
      resolve()
    }
 ...
 */
const fileUpload = ({message,  okButton, cancelButton}) => {
...
// alert confirm용 모달
const infoModal = new bootstrap.Modal(document.querySelector('#infoModal'), {backdrop: 'static', keyboard: false});
...
async function alertMessage(title, message, bsColor = 'primary') {
  ...
  //console.log('alertMessage infoModal', infoModal._isShown, infoModal._isTransitioning, infoModal);
    ...
    return new Promise((resolve, reject) => {
      okButton.addEventListener('click', () => {
        if(options?.okButton?.callback){
          if(!options.okButton.callback()) return false;
        }
      ...
      cancelButton?.addEventListener('click', () => {
        ...
        if(options?.cancelButton?.callback){
          if(!options.cancelButton.callback()) return false;
        }
...
export {Question, chat, answer, questionWithInput, fileUpload, alertMessage, confirmMessage, customConfirmMessage};
