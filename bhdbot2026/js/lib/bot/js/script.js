...
import { alertMessage, answer, chat, confirmMessage, fileUpload, questionWithInput } from "./chat.js";

import { Question, TgrInfo, AgrInfo, IsrPrd, StoredData, getHashidsHelper, loading, formatSsn, getSbcAmt } from "./bhdSlcScript.js";
...
export function processor() {
  ...
  // modal 초기화
  window.infoModal = new bootstrap.Modal(this.$refs.infoModal, { backdrop: 'static', keyboard: false });
  ...
  return {
    ...
    async init() {
      ...
      console.log('init', this.current, this.$refs, Object.keys(this.selected).length);

      // locading 설정
      this.$watch('this.isLoading', loading)
      ...
      if (this.current === -2) {
        ...
        //if (!this.testMode) {
        this.hashidsHelper = getHashidsHelper(this.token);
        //}
        ...
        try {
          ...
          this.throwException('오류가 발생했습니다. 잠시 후 다시 시도해 주세요.<br><br>문의 사항은 공단(☎1588-4321)로 연락 바랍니다.<br><br>')
...
문의 사항은 공단(☎1588-4321)로 연락 바랍니다.`, { stack: `pctime=${new Date().toLocaleDateString()}` })
    ...
    welcome() {
      ...
      modal._element.addEventListener('hidden.bs.modal', () => {
        ...
        console.log('moment locale', moment.locale(), moment().format('LLLL'))
   ...
   수집·이용 및 제3자(보험사) <mark>제공 동의 절차가 완료</mark>되어야만 최종 가입이 완료됩니다.</span>
        ...
        alertMessage(`${this.bseYr}년도 보험선택${this.bhdSlcChnYn ? ' 의료비 보장 보험 조정' : ''} 안내`
          , welcomeModalMessage, 'info').then(() => {
    ...
    /**
     * 채팅 시작 시 채팅창과 breadcrumb을 초기화 하고 시간 표시
     */
    chatStartTime() {
      ...
      this.$refs.messages.appendChild(_div);
...
미가입 대상자에 해당하십니까?`)
  ...
  ${optionTxt}`, { delay });
    ...
    doQuestion() {
      ...
      if (this.current >= this.questions.length || thisQuestion.data.isrrClCd === '3') {
        ...
        if (this.current >= this.questions.length) {
          ...
          if (this.chldInfo && this.chldInfo.length && this.chldInfo.length > this.selected[thisQuestion.data.isrPrdCd].length) {
     ...
     * 3 변경 여부
     * 4 배정정보 불러올건지 체크
     * @param step {0|1|2|3|4} - 0: 가입여부, 1: 성명, 2: 주민번호, 3: 변경 여부, 4 배정정보 불러올건지 체크
     */
  ...
  <ul>
    ...
    <li>부부 공무원의 경우 본인의 보험에 가입하는 것이 원칙</li>
        ...
        const checkName = (evt) => {
          ...
          if (!spsName) {
            markingInvalid(input, true, `배우자의 성명을 입력해주세요`)
            return false;
          }

          // 성명 체크
          if (!validateName(spsName)) {
            markingInvalid(input, true, `배우자의 성명을 확인해 주세요
성명은 영문 한글 공백만 가능합니다.`)
            return false;
          }
        ...
        input.addEventListener('keyup', evt => {
          ...
          if (rrno?.length === 13) {
            ...
            // 주민번호 형식 체크
            if (!validateSsn(rrno)) {
              markingInvalid(evt.target, true, '올바른 주민번호가 아닙니다.')
              return false;
            }

            if (rrno === this.pseInfo.rrno) {
              markingInvalid(evt.target, true, '배우자의 주민번호가 동일합니다.')
              return false;
            }
            ...
            if (tgrSxClCd === this.pseInfo.sxClCd) {
              markingInvalid(evt.target, true, '배우자의 성별이 동일합니다.')
              return false;
            }
            if (tgrAg < 18) {
              markingInvalid(evt.target, true, '배우자의 주민번호가 미성년자입니다.')
              return false;
            } else {
    ...
    <li>단, 장애가 있는 자녀는 미혼인 경우에 한해 연령에 관계없이 가입이 가능합니다.<b>(증빙서류 필요)</b></li>
        ...
        const checkName = (evt) => {
          ...
          if (!validateName(childName)) {
            ...
            markingInvalid(input, true, `자녀의 성명을 확인해 주세요
성명은 영문 한글 공백만 가능합니다.`)
            input.focus();
            return false;
          }
          ...
          if (storedName && childName === storedName) {
            ...
            if (Object.values(this.selected).flat().find(p => p?.tgrFnm === childName)) {
              markingInvalid(input, true, '이미 등록되어 있는 성명 입니다.')
              input.focus();
              return false;
            }
        ...
        const checkChildRrno = (input) => {
          ...
          if (rrno?.length !== 13) {
            markingInvalid(input, true, '올바른 주민번호가 아닙니다. 13자리를 입력해주세요')
            return false;
          }
          ...
          // 주민번호 형식 체크
          if (!validateSsn(rrno)) {
            markingInvalid(input, true, '올바른 주민번호가 아닙니다.')
            return false;
          } else {
            markingInvalid(input, false)
          }
          ...
          if (rrno === this.pseInfo.rrno) {
            markingInvalid(input, true, '자녀의 주민번호가 동일합니다.')
            return false;
          }

          if (this.spsInfo?.rrno === rrno) {
            markingInvalid(evt.target, true, '자녀의 주민번호와 배우자의 주민번호가 동일합니다.')
            return false;
          }
          ...
          if (!storedRrno || rrno !== storedRrno) {
            ...
            if (Object.values(this.selected).flat().find(p => p?.tgrRrno === rrno)) {
              markingInvalid(input, true, '이미 등록되어 있는 주민번호 입니다.')
              return false;
            }
          }
          ...
          if (tgrAg < 0) {
            markingInvalid(input, true, '자녀의 나이가 0세 미만 입니다.')
            return false;
          }

          if (tgrAg >= this.pseInfo.age) {
            markingInvalid(input, true, '자녀의 나이가 부모의 나이 보다 많습니다.')
            return false;
          }
...
<br>생년월일: <strong>${moment(birthday).format('YYYY년 M월 D일')}</strong>`)
  ...
  <b class="text-danger">미동의시 가입불가</b>`, this.chldInfo[index])
                        ...
                        fileUpload(({ message, okButton, cancelButton, bootstrap }));
                        return;
