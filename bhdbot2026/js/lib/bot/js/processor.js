...
export function processor() {
  ...
  return {
    ...
    mobileYn: 'N',
    /**
     * {Date|number} - 시작하고 보험데이터를 가져오기까지 걸린 시간
     */
    loadtime: null,
    /**
     * {Date|number} - 시작한 시간
    ...
    testMode: false,
    /**
     * {boolean} - 보험선택 완료 되었었는가?
     */
    dcnYn: false,
    /**
     * {boolean} - 의료보험 조정 기간인가?
     */
    bhdSlcChnYn: false,
    spsEssYn: true,
    storedData: null,
    ...
    async getIsrData() {
      ...
      if (this.testMode) {
        ...
        this.questions = rtnList.map(d => new Question({
          ...
          'message': `${d.essYn ? '<span class="text-success bi bi-check2-square"> 필수</span>' 
            : '<span class="text-primary bi bi-bag-plus"> 선택</span>'} 
<b>${d.isrPrdNm}</b> ${this.bhdSlcChnYn?'보험 선택 내역 입니다.':'보험을 선택해 주세요'}`,
...
console.log('this.bhdSlcChnYn', this.bhdSlcChnYn)
          ...
          this.questions = jsn.rtnList
            ...
            .map(d => new Question({
              'short_title': d.isrPrdNm,
              'message': `${d.essYn?'<span class="text-success bi bi-check2-square"> 필수</span>'
                  :'<span class="text-primary bi bi-bag-plus"> 선택</span>'} <b>${d.isrPrdNm}</b>
 ${(this.bhdSlcChnYn && !this.checkIL002(d.isrPrdCd) )? '보험 선택 내역 입니다.' : '보험을 선택해 주세요'}`,
              'optionFunc': this.makeIsrPrdOption.bind(this),
              'data': d
            }));
...
문의 사항은 공단(☎1588-4321)로 연락 바랍니다.`, {stack: `pctime=${new Date().toLocaleDateString()}`})
  ...
  문의 사항은 공단(☎1588-4321)로 연락 바랍니다.`, {stack: `pctime=${new Date().toLocaleDateString()}`})
     ...
     * @return {boolean}
     */
    async checkName(input, storedName)  {
...
성명은 영문 한글 공백만 가능합니다.`;
      ...
      if (this.pseInfo.usrFnm === name) {
        ...
        // 드물게 부부가 이름이 같은 경우가 있다.
        // 진짜 맞는지 확인 하고 넘어가자  25. 8. 11. choihunchul
        if(input.id === 'spsFnm'){
          ...
          const r = await Swal.fire({
            ...
            focusConfirm: false,
            didOpen(){
console.debug('didOpen')
            },
            willClose(){
console.debug('willClose')
            },
          }).then(r=>{
...
console.debug('flat=========', Object.values(this.selected).flat().map(s=>s.tgrFnm).join(','))

      let found = Object.values(this.selected).flat().find(p => p?.tgrFnm === name);
     ...
     * @return {boolean}
     */
    async checkFmlSsn(input, isrrClCd, storedRrno) {
      ...
      if (rrno?.length === 13) {
        ...
        if (isrrClCd === '1') {
          ...
          if (tgrAg < 19) {
            ...
            // 배우자 배정정보 체크
            const checked = await this.checkSpousePublicOfficial(rrno);
            ...
            if(checked){
                ...
                input.classList.remove('is-valid', 'animate__animated', 'animate__headShake');
                input.classList.add('is-invalid', 'animate__animated', 'animate__headShake');
                feedbackEl.innerText = '배정 대상자입니다.';
                input.focus();
                return false;
              }
      ...
      if (question.data.isrrClCd === '1' && !this.spsInfo?.usrFnm) {
        ...
        if(this.bhdSlcChnYn){
          ...
          if(this.spsEssYn){
            ...
            if (this.questions.find(q => q.data.isrrClCd === '1' && q.data.essYn)) {
...
console.debug('not _confirm', _confirm)
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="text-start ps-4">${formatSsn(rrno, false)}</td>
    </tr>
    <tr>
      <th scope="row">휴대폰</th>
...
console.debug('this.fmlInfo', res)
  ...
  <div class="col-md-6" 
    x-data="{
        ...
        checkName: async (el)=>{
           if(!this.isComposing){
            ...
            const valid = await checkName(el, spsInfo?.usrFnm)
            if(valid){
    ...
    <input type="text" id="spsFnm" class="form-control" x-ref="spsFnm" required tabindex="1"
      ...
      @keyup.enter="$refs.spsRrno.focus()"
  ...
  <div class="col-md-6" x-data="{
      checkRrno: async()=>{
        const valid = await checkFmlSsn($el, '1', spsInfo?.rrno)
...
<button class="w-25 btn btn-lg btn-danger bi bi-x-circle-fill"  tabindex="6" name="cancelButton">취소</button>`,
          okButton: ({
            callback: async () => {
              ...
              if(!spsFnm.classList.contains('is-valid')){

                const checkedName = await this.checkName(spsFnm, this.spsInfo?.usrFnm);
                if(!checkedName){
                  spsFnm.focus()
                  return false;
                }
              ...
              const b = await this.checkFmlSsn(spsRrno, '1', this.spsInfo?.rrno);
              if(!b){
                spsRrno.focus()
                return false;
              }
    ...
    async validateChildRow(tr) {
      ...
      const b = await this.checkName(childFnm, storedFnm);
console.debug('checkName b', b)
...
console.log('childFile.dataset.storedFile', childFile.dataset.storedFile)
    ...
    appendChild(child) {
      ...
      console.log('appendChild', child, 'bornYear', bornYear, 'tgrSxClCd', tgrSxClCd, 'birthday', birthday)

      const overChldIsrAgRstcYr  = child && bornYear > this.wlfInst.chldIsrAgRstcYr

      const childName = child?.usrFnm?child.usrFnm:''
      const childRrno = child?.rrno?child.rrno:''

      _row.innerHTML = `
        <th scope="row" x-text="parseInt($el.parentElement.rowIndex/2)+1" rowSpan="2" class="align-middle"></th>
        <td x-data="{
            checkName: async (el)=>{
               const storedName =  '${childName}'
               
console.log('storedName', storedName, el.value)               
               if(!el.value) return;
            
               const valid = await checkName(el, storedName)               
                if(valid){
                  
                  if(Array.from($el.closest('tbody')
                      .querySelectorAll('input[name=childFnm]')).find(i=>i.value === $el.value && i !== $el )){
                    el.classList.remove('is-valid')
                    el.classList.add('is-invalid', 'animate__animated', 'animate__headShake');
        
                    el.nextSibling.innerText = '이미 등록되어 있는 성명 입니다.';
                  }else{
                    //document.querySelector('button[name=okButton]').click()
                  }
                }
        ...
        <td x-data="{
              async checkChildRrno(el){
              
console.group('checkChildRrno', 'el', el)
                const rrno = el.value;
console.log('rrno', rrno)
              
                if(!rrno) return;
                
                const valid = checkFmlSsn(el, '3', rrno)
                
                if(valid){
      
                  const _td = el.parentElement
console.debug('_td', _td)
                  const _tr = el.parentElement.parentElement
console.debug('_tr', _tr)
...
console.debug('fileEl', fileEl, 'nextElement', nextElement)
                  ...
                  if (parseInt(bornYear) < parseInt(wlfInst.chldIsrAgRstcYr)) {
                    ...
                    const r = await Swal.fire({
                      icon: 'warning',
                      title: '증빙서류 등록 안내',
                      html: '<strong>' + wlfInst.chldIsrAgRstcYr + '년 1월 1일</strong> 이전 출생인 경우 장애가 있는 자녀만 가입이 가능하며,'
                    ...
                    // 입력정보 reset  
                      ...
                      const childFnm = _tr.querySelector('input[name=childFnm]');
                      childFnm.value = ''
                      childFnm.classList.remove('is-valid')
                      
                      const childRrno = _tr.querySelector('input[name=childRrno]')
                      childRrno.value = ''
                      childRrno.classList.remove('is-valid')
...
console.groupEnd('checkChildRrno end', rrno, 'el', el)
      ...
      const childOver14 = child && calcSsn(child.rrno, this.bseYr, true).tgrAg >=14;

      if(!childOver14) _row2.classList.add('visually-hidden')
        ...
        <button class="btn btn-success bi bi-plus-lg"
          ...
          x-init="()=>new bootstrap.Tooltip($el)"
           @click="()=>{
              ...
              setTimeout(() => {
                document.querySelector('input[name=childFnm]').focus()
              }, 400)
...
<b class="text-danger">미동의시 가입불가</b></li>
      ...
      <b>증빙서류</b>(복지카드 사본, 장애인 증명서 등)가 필요합니다.</li>
          ...
          onLoaded: () => {
           ...
           /* setTimeout(() => {
              console.log("document.querySelector('input[name=childFnm]:last-child)", document.querySelector('input[name=childFnm]'))
              document.querySelector('input[name=childFnm]').focus()
            }, 400)*/
          },
