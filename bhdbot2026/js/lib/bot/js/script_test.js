...
export const unboxingToken = (token, TgrInfo) => {
  ...
  console.debug('pako', pako)

  const myData = JSON.parse(pako.inflate(nxtUtil.base64ToByte(token), { to: 'string' }));

  console.debug('myData', myData)

  const { tgrSxClCd, tgrAg } = nxtUtil.calcSsn(myData.pseInfo.pseRrno, myData.bseYr)
  ...
  const fmlInfo = myData.fmlInfo?.map(f => {
    const fml = new TgrInfo(f.name, f.rrno, '', '', f.sxClCd, f.age, f.isrrClCd);
    fml.cellPhoneNo = f.tel
